...
def conv_files(paths, result_path, out_path, language, nlp):
    ...
    for path in paths:  # for cross_ & cross_intra- annotations folders
        dirs = os.listdir(path)
        cnt = 0

        for topic_dir in dirs:  # for each topic folder
            ...
            if "_cross-doc_annotation" in path:
                ...
                annotation_prefix = "cross_"  # the prefix to add before coref_chain
            else:
                topic_name = str(cnt) + "MEANTIME"
            ...
            for topic_file in tqdm(topic_files):
                ...
                for elem in root:
                    try:
                        ...
                        # do not overwrite conll previously generated ("cross" is processed first)
                        if annotation_prefix == "cross_":
                            if elem.tag == "token" and len(conll_df.loc[(conll_df[TOPIC_SUBTOPIC] == t_subt) &
                                                                        (conll_df[DOC_ID] == topic_name) &
                            ...
                            if len(tokens):
                                ...
                                if mention_head_id not in sent_tokens:  # also "if is None"
                                    if str(t_subt) + "_" + str(mention_text) not in need_manual_review_mention_head:
                                        ...
                                        with open(os.path.join(out_path, MANUAL_REVIEW_FILE.replace(".json",
                                                                                                    "_" + language + ".json")),
                                                  "w",
            ...
            for chain_index, (chain_id, chain_vals) in enumerate(coref_dict.items()):
                if chain_vals.get("mentions") is not None and chain_id != "":
                    for m in chain_vals["mentions"]:
                        ...
                        # if the mentions has not been processed for another path (cross, intra) already, just add it
                        if not any(n[MENTION_ID] == mention_id for n in entity_mentions) and not any(
                                n[MENTION_ID] == mention_id for n in event_mentions):
            ...
            # remap the local mentions with the updated cross/intra prefixes
            entity_mentions_local, event_mentions_local = [], []
            for m in entity_mentions:
                if m[TOPIC_SUBTOPIC].split("/")[0] == t_subt.split("/")[0]:
                    entity_mentions_local.append(m)
            for m in event_mentions:
                if m[TOPIC_SUBTOPIC].split("/")[0] == t_subt.split("/")[0]:
                    event_mentions_local.append(m)
            ...
            for (topic_local), topic_df in conll_topic_df.groupby(by=[TOPIC_SUBTOPIC]):
                ...
                for (sent_id_local), sent_df in topic_df.groupby(by=[SENT_ID], sort=[SENT_ID]):
                    np.savetxt(os.path.join(MEANTIME_PARSING_FOLDER, "tmp.txt"), sent_df.values, fmt='%s',
                               delimiter="\t",
                               encoding="utf-8")
            ...
            try:
                ...
                LOGGER.info(
                    f"Amount of mentions in this topic: {str(len(event_mentions_local + entity_mentions_local))}")
    ...
    LOGGER.info(
        ...
        MANUAL_REVIEW_FILE.replace(".json",
                                   "_" + language + ".json - Total: " + str(len(need_manual_review_mention_head))))
